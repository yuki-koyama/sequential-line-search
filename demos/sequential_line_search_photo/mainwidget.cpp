...
void MainWidget::paintEvent(QPaintEvent *event)
{
    ...
    constexpr double base_scale = 4.0;
    
    // Draw setting
    const double slider_base_width = base_scale;
    const double slider_back_width = 2.0 * slider_base_width;
    const double knob_radius       = base_scale;
    ...
    const double dot_radius      = base_scale;
        ...
        for (int pix_y = 0; pix_y < h; ++ pix_y)
        {
            ...
            switch (content)
            {
                ...
                case Content::ExpectedImprovement:
                    val(pix_x, pix_y) = (core.regressor.get() != nullptr) ? acquisition_function::calculateExpectedImprovedment(*core.regressor, x) : 0.0;
                    break;
                default:
                    break;
            }
